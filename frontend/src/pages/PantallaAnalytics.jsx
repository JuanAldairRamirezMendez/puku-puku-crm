...
function MlTraining() {
  ...
  useEffect(() => {
    api.entrenarStatus().then(setStatus).catch(() => {});
  }, []);
  ...
  async function handleEntrenar() {
    ...
    setError('');
    try {
      const res = await api.entrenarMl();
    ...
    <div style={{ marginTop: 24 }}>
      <div className="card">
        <div className="segmentacion-header">
          <h3>Entrenamiento ML — Churn Prediction</h3>
          <div className="segmentacion-controls">
            <button className="btn-principal" onClick={handleEntrenar} disabled={cargando}>
              {cargando ? 'Entrenando…' : status?.entrenando ? 'En curso…' : 'Entrenar modelo'}
            </button>
          </div>
        </div>
        ...
        {status && !status.entrenando && (
          <div style={{ marginTop: 12, fontSize: '0.85rem' }}>
            {status.last_trained && (
              <p style={{ color: 'var(--color-brown-700)', marginBottom: 8 }}>
                Último entrenamiento: {new Date(status.last_trained).toLocaleString('es-PE')}
              </p>
            )}

            {status.targets_met && (
              <div style={{ display: 'flex', gap: 16, marginBottom: 12 }}>
                <span style={{ color: status.targets_met.accuracy_80pct ? 'var(--color-success)' : 'var(--color-danger)' }}>
                  Accuracy ≥ 80%: {status.targets_met.accuracy_80pct ? '✅' : '❌'}
                </span>
                <span style={{ color: status.targets_met.roc_auc_085 ? 'var(--color-success)' : 'var(--color-danger)' }}>
                  ROC-AUC ≥ 0.85: {status.targets_met.roc_auc_085 ? '✅' : '❌'}
                </span>
              </div>
            )}

            {status.metrics && (
              <div className="metricas-grid" style={{ gridTemplateColumns: 'repeat(5, 1fr)' }}>
                <div className="card metrica-card" style={{ padding: '8px 12px' }}>
                  <span className="metrica-valor" style={{ fontSize: '1.1rem' }}>{status.best_model || '-'}</span>
                  <span className="metrica-label">Mejor modelo</span>
                </div>
                <div className="card metrica-card" style={{ padding: '8px 12px' }}>
                  <span className="metrica-valor" style={{ fontSize: '1.1rem' }}>{(status.metrics.accuracy * 100).toFixed(1)}%</span>
                  <span className="metrica-label">Accuracy</span>
                </div>
                <div className="card metrica-card" style={{ padding: '8px 12px' }}>
                  <span className="metrica-valor" style={{ fontSize: '1.1rem' }}>{(status.metrics.roc_auc * 100).toFixed(1)}%</span>
                  <span className="metrica-label">ROC-AUC</span>
                </div>
                <div className="card metrica-card" style={{ padding: '8px 12px' }}>
                  <span className="metrica-valor" style={{ fontSize: '1.1rem' }}>{(status.metrics.f1 * 100).toFixed(1)}%</span>
                  <span className="metrica-label">F1-Score</span>
                </div>
                <div className="card metrica-card" style={{ padding: '8px 12px' }}>
                  <span className="metrica-valor" style={{ fontSize: '1.1rem' }}>{(status.metrics.precision * 100).toFixed(1)}%</span>
                  <span className="metrica-label">Precisión</span>
                </div>
              </div>
            )}
            ...
            {status.model && (
              <details style={{ marginTop: 8 }}>
                <summary style={{ cursor: 'pointer', fontSize: '0.82rem', color: 'var(--color-brown-700)' }}>
                  Detalles del modelo ({status.model.model_type}, {status.model.n_estimators} árboles, {status.model.n_features} features)
                </summary>
                <pre style={{ fontSize: '0.72rem', marginTop: 6, maxHeight: 200, overflow: 'auto', background: 'var(--color-cream)', padding: 8, borderRadius: 6 }}>
                  {JSON.stringify(status.model, null, 2)}
                </pre>
              </details>
            )}
          </div>
        ...
        {cargando && (
          <p className="cargando" style={{ marginTop: 12 }}>Entrenando modelo de churn con 8000 clientes sintéticos…</p>
        )}

        {!status && !cargando && !error && (
          <p className="sin-datos" style={{ marginTop: 12 }}>
            Presiona "Entrenar modelo" para ejecutar el pipeline completo de Machine Learning (11 modelos, validación cruzada, exportación a JSON).
          </p>
        )}
      </div>
    </div>
